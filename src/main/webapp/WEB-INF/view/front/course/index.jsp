...
<body class="w100">
	...
	<div id="app">
		<!--banner图-->
		<div class="banner"
			style="background-image: url('${pageContext.request.contextPath }/res/img/banner-${subjectId}.jpg')"></div>
		...
		<div class="classify">
			...
			<c:forEach items="${courses }" var="course">
				<div class="section">
					...
					<ul>
						<c:forEach items="${course.videoSet }" var="video" varStatus="status">
							<li class="section-main" onclick="getVideo(${video.id})">
							<div class="thum" style="background-image: url(${video.videoImageUrl})"></div>
								<p><span>${status.count }. </span>${video.videoTitle }</p>
								<div class="classify-v-info">
									<span class="count" title="观看次数">
									<img src="${pageContext.request.contextPath }/res/img/count.png" alt="">${video.videoPlayTimes }</span>
									<span class="duration" title="视频时长">
										<img src="${pageContext.request.contextPath }/res/img/player.png" alt="">${video.videoLengthStr }</span>
								</div>
							</li>
						</c:forEach>
					</ul>
	...
	<script type="text/javascript">
		function getVideo(id){
			...
			location.href='${pageContext.request.contextPath }/front/video_index?videoId='+id+'&subjectId='+subjectId;
		}
	</script>

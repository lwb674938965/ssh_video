...
<div class="catalog">
	<div class="container">
		...
		<c:forEach items="${list.videoSet}" var="video" varStatus="status">
			<div class="chapter">
				<p class="biaoti"><span>${status.count }. </span><a href="${pageContext.request.contextPath }/front/video_index?videoId=${video.id}&subjectId=${list.subject.id}">${video.videoTitle}</a></p>
				<p class="lecturer">${video.videoDescr}</p>
				<p class="lecturer">讲师：${video.speaker.speakerName}</p>
				<div class="v-info">
					<span class="count"><img src="${pageContext.request.contextPath }/res/img/count.png" alt="">${video.videoPlayTimes}</span>
					<span class="duration"><img src="${pageContext.request.contextPath }/res/img/player.png" alt="">${video.videoLengthStr}</span>
				</div>
			</div>
		</c:forEach>
